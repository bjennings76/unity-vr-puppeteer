...
namespace Utils {
	public static class UnityUtils {
		...
		public static List<Type> GetChildTypes<T>() {
			...
			foreach (var assem in AppDomain.CurrentDomain.GetAssemblies()) foreach (var type in assem.GetTypes()) if (typeof(T).IsAssignableFrom(type) && type != typeof(T) && !type.IsAbstract) derivedTypes.Add(type);
		...
		public static string GetRelativePath(FileSystemInfo start, FileSystemInfo target) {
			...
			var uri1 = new Uri(target.FullName);

			if (start is FileInfo) { uri2 = new Uri(start.FullName); }
			else {
				var folderName = start.FullName;
		...
		public static GameObject GetGameObject(Object thing) {
			var t = thing as Transform;
			if (t) return t.gameObject;
			var c = thing as Component;
		...
		private static string GetPath(Transform root, Transform target) {
			var t = target;
			var path = t.FullName(Extensions.FullNameFlags.UniqueName);
		...
		public static Dictionary<T, Transform> Orphan<T>(IEnumerable<T> things) where T : Object {
			var orphans = new Dictionary<T, Transform>();
			foreach (var thing in things) {
				var go = GetGameObject(thing);
		...
		public static void Readopt<T>(Dictionary<T, Transform> orphans) where T : Object {
			var active = orphans.Where(kvp => kvp.Key && kvp.Value).ToList();

			foreach (var kvp in active) {
				var go = GetGameObject(kvp.Key);
		...
		public static Camera Camera {
			get {
				if (!s_Camera) s_Camera = Camera.allCameras.FirstOrDefault(c => c.name == "MLCamera0") ?? Camera.main ?? Camera.current ?? Camera.allCameras.FirstOrDefault();
				return s_Camera;
			}
		}
		...
		private static Bounds GetBoundsOfChildren(params Transform[] components) {
			...
			components.ForEach(c => {
				var b = new Bounds(c.transform.position, Vector3.one);
				if (!foundBounds) {
					foundBounds = true;
					bounds = b;
				}
		...
		public static void DelayAction(float duration, Action onComplete, object target = null) {
			...
			var sequence = DOTween.Sequence().SetDelay(duration).OnComplete(() => onComplete());
			if (target != null) sequence.SetTarget(target);
		}
		...
		public static Type GetFirstTypeByName(string typeName) {
			...
			var typeQualification = typeName.Split('.');
			var name = typeQualification[typeQualification.Length - 1];
			var qualifiedName = typeQualification.Length > 1;

			foreach (var a in AppDomain.CurrentDomain.GetAssemblies()) {
				var assemblyTypes = a.GetTypes();
				for (var j = 0; j < assemblyTypes.Length; j++) if (assemblyTypes[j].Name == name) if (!qualifiedName || assemblyTypes[j].FullName == typeName) return assemblyTypes[j];
			}
