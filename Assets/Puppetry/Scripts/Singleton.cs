...
public class Singleton<T> : MonoBehaviour where T : MonoBehaviour {
	private class SingletonDebug : XDebug.Channel<SingletonDebug> {
		...
		get {
			if (s_Instance == null)
				if (s_Quitting) {
					...
					if (s_Instance == null) {
						var container = new GameObject {name = typeof(T) + " Container"};
	...
	public static bool Exists() { return s_Instance != null; }
}